    ...
    protected function fillResult()
    {
        ...
        if ($obCache->startDataCache(60 * 60 * 24)) {
            ...
            while ($ar = $rsElems->Fetch()) {
                $arItem = [];
                $arSrc = \CFile::ResizeImageGet($ar['PREVIEW_PICTURE'], ['width' => 800, 'height' => 800], BX_RESIZE_IMAGE_EXACT, false, false, false, 100);
                if (!empty($arSrc['src'])) {
                    $arItem['BIG'] = $arSrc['src'];
                }
                $arSrc = \CFile::ResizeImageGet($ar['PREVIEW_PICTURE'], ['width' => 400, 'height' => 800], BX_RESIZE_IMAGE_EXACT, false, false, false, 100);
                if (!empty($arSrc['src'])) {
