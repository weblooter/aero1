...
class TranslateListComponent extends Translate\ComponentBase
{
		...
		if (empty($result))
		{
			...
			*/
			...
			$result[] = array(
				...
				'default' => ($this->action == self::ACTION_SEARCH_PHRASE),
				'sticked' => true,
				'prevent_default' => false,
	...
	private function executeFileListAction()
	{
		...
		// view mode
		$hideEmptyFolders = ($paramsIn['HIDE_EMPTY_FOLDERS'] === true);
		$showDiffLinks = ($paramsIn['SHOW_DIFF_LINKS'] === true);

		$this->arResult['IS_INDEXED'] = $isIndexed = false;

		// top folder
		$topIndexPath = $this->detectTopIndexPath();
		if ($topIndexPath instanceof Translate\Index\PathIndex)
		{
			$this->arResult['IS_INDEXED'] = $isIndexed = $topIndexPath->getIndexed();
		...
		$this->loadActualFileData($isIndexed && $hideEmptyFolders);
		...
		$this->arResult['GRID_DATA'] = array_merge($this->arResult['GRID_DATA'], $this->dirData, $this->fileData);
	}
	...
	/**
	 * Loads actual data from disk files.
	 *
	 * @param boolean $hideEmptyFolders Hhide empty folders.
	 *
	 * @return void
	 */
	private function loadActualFileData($hideEmptyFolders = false)
	{
		...
		{
			if (!in_array(strtoupper($langId).'_LANG', $gridColumns))
			{
				continue;
			}
		...
		foreach ($iterateDirectory($topFolder->getPath(), $this->path, $isTopLang) as $entry)
		{
			...
			if ($hideEmptyFolders && $entry['columns']['IS_DIR'])
			{
				if (!isset($this->indexData[$pathId]))
				{
					continue;
	...
	/**
	 * Appends actual file data by date from index.
	 *
	 * @param boolean $loadPathsDiff Load path data for link.
	 *
	 * @return void
	 */
	private function loadIndexFileData($loadPathsDiff = false)
	{
		$paramsIn =& $this->getParams();
		// top folder
		$topIndexPath = $this->detectTopIndexPath();
		if ($topIndexPath instanceof Translate\Index\PathIndex)
		{
			try
			{
				...
				foreach ($languageUpperKeys as $langId => $langUpper)
				{
					if (!in_array("{$langUpper}_LANG", $gridColumns))
					{
						continue;
					}
					...
					$runtime[] = new Main\ORM\Fields\Relations\Reference(
						...
						Main\ORM\Query\Join::on('ref.PATH_ID', '=', 'this.PATH_ID')->where('ref.LANG_ID', '=', $langId),
						array('join_type' => 'LEFT')
					);

					if ($langId == $paramsIn['CURRENT_LANG'])
					{
						$select["{$alias}_CNT"] = new Main\ORM\Fields\ExpressionField(
							"{$alias}_CNT",
					...
					else
					{
						...
						$select["{$alias}_DIFF"] = new Main\ORM\Fields\ExpressionField(
							"{$alias}_DIFF",
							"@{$alias}_CNT - @ETHALON_CNT"
						);
					}
				}
				...
				if ($loadPathsDiff)
				{
					...
					/** @var Main\ORM\Query\Result $cursor */
				$cursor = Translate\Index\Internals\PathIndexTable::getList(array(
					'select' => array_merge(array(
						'PARENT_PATH' => 'TOP.PATH',
						'PATH_ID' => 'ID',
						'OBLIGATORY_LANGS' => 'OBLIGATORY_LANGS',
					), $select),
					'runtime' => $runtime,
					'filter' => array(
						'=IS_DIR' => 'N',
						'>=LEFT_MARGIN' => $topIndexPath->getLeftMargin(),
						'<=RIGHT_MARGIN' => $topIndexPath->getRightMargin(),
						'=TOP.IS_DIR' => 'Y',
						'=TOP.PARENT_ID' => $topIndexPath->getId(),
					),
				));

				$ethalonCnt = strtoupper($paramsIn['CURRENT_LANG']). '_CNT';
				...
				foreach ($cursor as $row)
				{
					...
					foreach ($languageUpperKeys as $langId => $langUpper)
					{
						...
						$aliasCnt =  "{$langUpper}_CNT";
						$aliasDiff =  "{$langUpper}_DIFF";

						$isObligatory = empty($row['OBLIGATORY_LANGS']) || strpos($row['OBLIGATORY_LANGS'], $langId) !== false;

						if ($langId == $paramsIn['CURRENT_LANG'])
						{
							if ((int)$row[$aliasCnt] > 0)
							{
								$this->indexData[$parentPath][$langId]['file_count']++;
								$this->indexData[$parentPath][$langId]['phrase_count'] += (int)$row[$aliasCnt];

								if (!$isObligatory)
								{
									$this->indexData[$parentPath][$langId]['file_excess'] ++;
									$this->indexData[$parentPath][$langId]['phrase_excess'] += (int)$row[$aliasCnt];
								}
							}
							continue;
						}
						...
						$deficiency = $excess = 0;
						if ($row[$aliasCnt] == 0 && (int)$row[$ethalonCnt] > 0)
						{
							if ($isObligatory)
							{
								$deficiency = (int)$row[$ethalonCnt];
								$this->indexData[$parentPath][$langId]['file_deficiency'] ++;
								$this->indexData[$parentPath][$langId]['phrase_deficiency'] += $deficiency;
							}
						}
						elseif ($row[$ethalonCnt] == 0 && (int)$row[$aliasCnt] > 0)
						{
							$this->indexData[$parentPath][$langId]['file_count'] ++;
							$this->indexData[$parentPath][$langId]['phrase_count'] += (int)$row[$aliasCnt];

							$excess = (int)$row[$aliasCnt];
							$this->indexData[$parentPath][$langId]['file_excess'] ++;
							$this->indexData[$parentPath][$langId]['phrase_excess'] += $excess;
						}
						elseif ((int)$row[$aliasCnt] > 0 && (int)$row[$ethalonCnt] > 0)
						{
							$this->indexData[$parentPath][$langId]['file_count'] ++;
							$this->indexData[$parentPath][$langId]['phrase_count'] += (int)$row[$aliasCnt];

							if ((int)$row[$aliasDiff] > 0 || !$isObligatory)
							{
								if ($isObligatory)
								{
									$excess = (int)$row[$aliasDiff];
									$this->indexData[$parentPath][$langId]['phrase_excess'] += $excess;
								}
								else
								{
							...
							elseif ((int)$row[$aliasDiff] < 0)
							{
								if ($isObligatory)
								{
									$deficiency = abs((int)$row[$aliasDiff]);
									$this->indexData[$parentPath][$langId]['phrase_deficiency'] += $deficiency;
								}
								else
								{
