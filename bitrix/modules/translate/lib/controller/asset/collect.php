	...
	implements Translate\Controller\ITimeLimit, Translate\Controller\IProcessParameters
{
	...
	public function run()
	{
		if ($this->isNewProcess)
		{
			...
			if (!$this->hasErrors())
			{
				$targetDirectory = new Translate\IO\Directory($this->tmpFolderPath);
				if ($targetDirectory->wipe() === false)
				{
					$this->addError(
						new Error(Loc::getMessage('TR_ERROR_DELETE_TARGET_FOLDER', array('#PATH#' => $this->tmpFolderPath)))
					);
				}
			}
			*/
