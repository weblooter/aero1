	...
	extends Index\Internals\EO_PathIndex_Collection
{
	...
	/**
	 * Switch on|off auto arrangement mode within inserting new nodes.
	 *
	 * @param bool $flag On|Off mode.
	 *
	 * @return void
	 */
	public function switchArrangeMode($flag)
	{
		$this->arrangeNewNodes = $flag;
	}
	...
	public function collect(Translate\Filter $filter = null, Translate\Controller\ITimeLimit $timer = null, Translate\Filter $seek = null)
	{
		...
		while ($pathLang = $cachePathLangRes->fetch())
		{
			...
			if ($timer !== null && $timer->hasTimeLimitReached())
			{
				if ($seek !== null)
				{
					$seek->nextPathLangId = (int)$pathLang['ID'];
				}
				break;
			}
		}
			...
			)
			{
					...
					if (!empty($childrenList))
					{
						foreach ($childrenList as $fullPath)
						{
							...
							if ($pathId === null)
							{
								...
								if ($this->arrangeNewNodes)
								{
									$newLeftMargin = Index\Internals\PathIndexTable::beforeInsertNode($parentId);
									$nodeData['LEFT_MARGIN'] = $newLeftMargin;
									$nodeData['RIGHT_MARGIN'] = $newLeftMargin + 1;
								}
				...
				if ($parentLangId === null && basename($parentFullPath) === 'lang')
				{
					...
					if (self::$useTranslationRepository)
					{
						...
						foreach (self::$translationEnabledLanguages as $langId)
						{
	 ...
	 * @return array|null
	 */
	private function constructAncestorsByPath($path)
	{
		...
		$pathRes = Index\Internals\PathIndexTable::getList([
			'select' => ['ID', 'DEPTH_LEVEL', 'IS_LANG', 'PATH'/*, 'LEFT_MARGIN', 'RIGHT_MARGIN'*/],
		...
		foreach ($pathParts as $part)
		{
			...
			if ($this->arrangeNewNodes)
			{
				$newLeftMargin = Index\Internals\PathIndexTable::beforeInsertNode($searchParentId);
				$nodeData['LEFT_MARGIN'] = $newLeftMargin;
				$nodeData['RIGHT_MARGIN'] =  ++ $newLeftMargin;
			}
	...
	private function getAncestors($nodeId, $topNodeId = -1)
	{
		...
		if ($nodeInx = $nodeRes->fetchObject())
		{
			...
			if ((int)$nodeInx->getParentId() > 0)
			{
				$nodeRes = Index\Internals\PathIndexTable::getList([
					'filter' => [
						'<=LEFT_MARGIN' => $nodeInx->getLeftMargin(),
						'>=RIGHT_MARGIN' => $nodeInx->getRightMargin(),
					],
	 ...
	 * @return self
	 */
	public function removeNonLang(Translate\Filter $filter = null)
	{
		$searchPath = isset($filter, $filter->path) ? $filter->path : '';
		if (!empty($searchPath))
		{
			$searchPath = Translate\IO\Path::replaceLangId($searchPath, '#LANG_ID#');
			...
			if ($pathTop = $pathStartRes->fetch())
			{
				Index\Internals\PathIndexTable::removeNonLang(array(
					'>=LEFT_MARGIN' => $pathTop['LEFT_MARGIN'],
					'<=RIGHT_MARGIN' => $pathTop['RIGHT_MARGIN'],
				));
			}
		}
		else
		{
			Index\Internals\PathIndexTable::removeNonLang();
		}
	...
	public function collectModuleAssignment(Translate\Filter $filter = null)
	{
		...
		if (!empty($searchPath))
		{
			$pathStartRes = Index\Internals\PathIndexTable::getList([
				...
				'select' => ['ID', 'PATH', 'LEFT_MARGIN', 'RIGHT_MARGIN'],
			]);
			if ($path = $pathStartRes->fetchObject())
			{
				...
				if (count($relPathParts) >= 3 && $relPathParts[0] == 'bitrix' && $relPathParts[1] == 'modules')
				{
					...
					if ($moduleId !== null)
					{
						Index\Internals\PathIndexTable::bulkUpdate(
							['MODULE_ID' => $moduleId],
							['>=LEFT_MARGIN' => $path->getLeftMargin(), '<=RIGHT_MARGIN' => $path->getRightMargin()]
						);
					}
				}
	...
	public function collectAssignment(Translate\Filter $filter = null)
	{
		...
		while ($pathModules = $pathModulesRes->fetch())
		{
			$pathList = Index\Internals\PathIndexTable::getList([
				...
				'select' => ['ID', 'PATH', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'MODULE_ID'],
			]);
			while ($modulePath = $pathList->fetchObject())
			{
				...
				foreach (Translate\ASSIGNMENT_TYPES as $assignmentId)
				{
					...
					$pathEntryRes = Index\Internals\PathIndexTable::getList([
						'filter' => [
							'=PATH' => $filterPaths,
							'>=LEFT_MARGIN' => $modulePath->getLeftMargin(),
							'<=RIGHT_MARGIN' => $modulePath->getRightMargin(),
						],
						'select' => ['ID', 'PATH', 'LEFT_MARGIN', 'RIGHT_MARGIN'],
					]);
					while ($path = $pathEntryRes->fetchObject())
