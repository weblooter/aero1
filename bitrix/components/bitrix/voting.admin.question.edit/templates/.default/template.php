...
\Bitrix\Main\UI\Extension::load(array("popup", "ajax", "ui.buttons", "ui.forms", "main.polyfill.core"));
...
$gridInstanceID = $arParams["ANSWER_PARAMS"]["INSTANCE_ID"];
$vote = $arResult["VOTE"];
	...
	$gridRow = array(
		...
		"actions" => array(
			...
			array(
				...
				"className" => "remove",
				"onclick" => "BX.Main.gridManager.getInstanceById('{$arParams["ANSWER_PARAMS"]["GRID_ID"]}').removeRow('{$row["ID"]}');"
			)
		),
...
?>
	...
	<form id="form_<?=$arParams["ANSWER_PARAMS"]["GRID_ID"]?>" method="POST" action="" enctype="multipart/form-data">
		<?=bitrix_sessid_post()?>
		...
		<input type="hidden" name="gridInstanceId" value="<?=$arParams["ANSWER_PARAMS"]["INSTANCE_ID"]?>" />
		...
		<div id="answer_grid_container" style="padding:12px 18px 12px 12px;">
			...
			?>
			<button id="add_regular_type_answer" class="ui-btn ui-btn-primary ui-btn-icon-add" onclick="BX.Vote.addAnswer('<?=$gridInstanceID?>'); return false;">+</button>
			<button id="add_text_type_answer" class="ui-btn ui-btn-primary ui-btn-light-border" onclick="BX.Vote.addTextAnswer('<?=$gridInstanceID?>'); return false;"><?= Loc::getMessage("VOTE_GRID_BUTTON_ADD_TEXT") ?></button>
		</div><?
		//endregion
		$tabControl->Buttons(
			array(
...
BX.ready(function() {
	...
	BX.Vote.setParams('<?=$arParams["ANSWER_PARAMS"]["INSTANCE_ID"]?>',
		{
			formId : 'form_<?=$arParams["ANSWER_PARAMS"]["GRID_ID"]?>',
			gridId: '<?=$arParams["ANSWER_PARAMS"]["GRID_ID"]?>',
