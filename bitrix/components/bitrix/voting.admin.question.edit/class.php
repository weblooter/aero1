	...
	class CVoteAdminQuestionEdit extends \CBitrixComponent
	{
		...
		public function executeComponent()
		{
			try
			{
				...
				Diag::getInstance()->collectDebugInfo($this->__name);
				...
				Diag::getInstance()->logDebugInfo($this->__name, $this->getName());
			}
			catch (Exception $e)
			{
		...
		protected function prepareParams()
		{
			...
			else if (($copyId = intval($this->request->get("COPY_ID"))) && $copyId > 0 && isset($questions[$copyId]))
			{
				...
				{
					$this->answers[] = (["ID" => "c".$answer["ID"]] + $answer);
				}
	...
	class VoteQuestionEditGrid
	{
		...
		public function merge(array $data)
		{
			foreach ($data as $d)
			{
				...
				$this->data[$d["ID"]]["NEW"] = "N";
				if ($d["ID"] > 0)
					$this->data[$d["ID"]]["SAVED"] = "Y";
				else
					$this->data[$d["ID"]]["SAVED"] = "N";
			}
			return $this->data;
		}
