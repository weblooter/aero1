...
window.BVoteConstructor.prototype.checkAnswerAdding = function(qId) {
	...
	if (this.a[qId].list) {
		if (this.a[qId].list.firstChild) {
			...
			do {
				BX.unbind(node.firstChild, "focus", this._do);
	...
	if (this.a[qId].node) { BX.show(this.a[qId].node); }
		...
		if (this.a[qId].list.lastChild) {
			BX.bind(this.a[qId].list.lastChild.firstChild, "focus", this._do);
		} else {
			BX.bind(nodeQuestion, "focus", this._do);
		}
	}
	return true;
};
...
window.BVoteConstructor.prototype._do = function(e)
{
	...
	var
		reg = /(add|del)\w/,
		node = e.target,
		className = reg.exec(node.className),
		res,
		ii,
		a,
		q,
