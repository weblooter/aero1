...
class CVoteAdminQuestions extends \CBitrixComponent
{
	...
	public function executeComponent()
	{
		try
		{
			Diag::getInstance()->collectDebugInfo($id = $this->randString(6));
			...
			$order = (empty($order["sort"]) ? ["ID" => "ASC"] : $order["sort"]);
			...
			Diag::getInstance()->logDebugInfo($id, $this->getName());
		}
		catch(Exception $e)
		{
	...
	protected function updateQuestion($id, $data)
	{
		if ($question = $this->vote->getQuestion($id))
		{
/*			$files = []; $files_del = ($this->request->getPost("FIELDS_del") ?: []);
			CAllFile::ConvertFilesToPost(($this->request->getFile("FIELDS") ?: []), $files);

			if (array_key_exists($ID, $files))
				$arFields += $files[$ID];
			if (array_key_exists($ID, $files_del))
			{
				$arFields += array_key_exists("IMAGE_ID", $arFields) ? [] : ["IMAGE_ID" => []];
